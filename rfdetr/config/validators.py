...
def validate_config_values(self):
    ...
    for field_name in required_fields:
        if not hasattr(self, field_name):
            ...
            if value is None:
                ...
                errors.append(
                    f"Required field '{field_name}' must be one of ['dinov2_windowed_small', 'dinov2_windowed_base'], got '{value}'"
                )
            elif field_name in [
                "hidden_dim",
                "patch_size",
                ...
                if not isinstance(value, list):
                    ...
                    if invalid_scales:
                        errors.append(
                            f"Required field '{field_name}' contains invalid scales: {invalid_scales}. "
    ...
    if supports_improvements:
        if not self.use_improvements:
            ...
            if self.num_encoder_layers != 0:
                errors.append(
                    f"num_encoder_layers should be 0 when use_improvements=False, got {self.num_encoder_layers}"
                )
            ...
            if hasattr(self, "improved_hidden_dim") and self.hidden_dim == self.improved_hidden_dim:
                errors.append(
                    f"hidden_dim={self.hidden_dim} matches improved_hidden_dim when use_improvements=False. "
            ...
            if self.num_encoder_layers == 0:
                errors.append(
                    f"num_encoder_layers should be > 0 when use_improvements=True, got {self.num_encoder_layers}"
                )
            ...
            if hasattr(self, "improved_hidden_dim"):
                ...
                if self.hidden_dim == original_dims["hidden_dim"]:
                    errors.append(
                        f"hidden_dim={self.hidden_dim} matches original value when use_improvements=True. "
        ...
        if self.num_encoder_layers != 0:
            errors.append(
                f"num_encoder_layers should be 0 for {self.__class__.__name__} (improvements not supported), got {self.num_encoder_layers}"
            )
        if self.use_cross_scale_fusion:
            errors.append(
                f"use_cross_scale_fusion should be False for {self.__class__.__name__} (improvements not supported)"
            )
    ...
    if self.sa_nheads > 0 and self.ca_nheads > 0:
        ...
        if self.ca_nheads < self.sa_nheads:
            errors.append(
                f"ca_nheads ({self.ca_nheads}) < sa_nheads ({self.sa_nheads}). "
    ...
    if self.hidden_dim > 0 and self.sa_nheads > 0:
        if self.hidden_dim % self.sa_nheads != 0:
            errors.append(
                f"hidden_dim ({self.hidden_dim}) is not divisible by sa_nheads ({self.sa_nheads}). "
    ...
    if self.hidden_dim > 0 and self.ca_nheads > 0:
        if self.hidden_dim % self.ca_nheads != 0:
            errors.append(
                f"hidden_dim ({self.hidden_dim}) is not divisible by ca_nheads ({self.ca_nheads}). "
                f"This may cause dimension mismatches in attention layers."
            )
