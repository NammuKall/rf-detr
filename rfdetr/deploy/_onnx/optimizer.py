...
class OnnxOptimizer:
    ...
    def insert_fmhca(self, node_q, node_kv, final_tranpose, mhca_idx, heads, num_dynamic=0):
        ...
        # Reshape dims
        shape = gs.Constant(
            f"Shape_KV_{mhca_idx}", np.ascontiguousarray(np.array([0, 0, heads, 2, dims_per_head], dtype=np.int64))
        )
        ...
        # Create fMHCA plugin
        fmhca = gs.Node(
            op="fMHCA", name=f"fMHCA_{mhca_idx}", inputs=[output_q, output_reshape], outputs=[output_final_tranpose]
        )
        # Insert node
        ...
        if num_dynamic > 0:
            ...
            reshape2_input1_shape = gs.Node(
                "Shape", f"Reshape2_fmhca{mhca_idx}_shape", inputs=[node_q.inputs[0]], outputs=[reshape2_input1_out]
            )
    ...
    def insert_fmha(self, node_qkv, final_tranpose, mha_idx, heads, num_dynamic=0):
        ...
        # Reshape dims
        shape = gs.Constant(
            f"Shape_QKV_{mha_idx}", np.ascontiguousarray(np.array([0, 0, heads, 3, dims_per_head], dtype=np.int64))
        )
